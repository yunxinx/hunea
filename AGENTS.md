...
- 涉及 `ratatui` 的实现、升级、重构或 API 选择时，优先使用 Context7 MCP 查询**最新官方文档与推荐写法**，理解后再修改代码，不要只凭旧记忆或零碎示例下判断。
- 如果准备采用某种写法，但它看起来可能不够自然、可能不是最佳实践、或可能带有明显迁移期痕迹，先使用可用的搜索工具查找官方文档、release note、recipe、examples 或公认最佳实践资料，再决定是否采用。
- 遇到“当前代码能工作，但写法可能不够干净/不够 idiomatic”的情况时，优先确认 Ratatui 与 Rust 社区的推荐方向；在行为等价且验证充分的前提下，可以采用更简洁、更自然的实现，而不必机械受限于仓库里的旧写法。
