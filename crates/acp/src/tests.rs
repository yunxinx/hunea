...
use crate::{
    AcpAgentIdentity, AcpAgentServerConfig, AcpAgentServerType, AcpPromptBlock,
...
fn resolve_custom_agent_session_command() {
    ...
    let config = acp_config_with_server(
        ...
        AcpAgentServerConfig {
            server_type: AcpAgentServerType::Custom,
            agent: String::new(),
...
fn acp_config_with_server(
    server_id: &str,
    server: AcpAgentServerConfig,
) -> AcpSessionCatalogConfig {
...
fn catalog_keeps_directly_launchable_agents_only() {
    ...
    let config = AcpSessionCatalogConfig {
        enabled: true,
        agent_servers,
    };

    let catalog = crate::AcpSessionCatalog::from_config(&config);
...
async fn initialize_agent_over_acp_transport_returns_agent_info() {
    ...
    let outcome = crate::initialize_agent_transport(client_transport)
        .await
...
async fn initialize_agent_command_reports_spawn_failure() {
    let command = crate::AcpSessionCommand {
    ...
    let error = crate::initialize_agent_command(&command)
        .await
