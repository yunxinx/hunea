...
use runtime_domain::session::{ChatMessage, ChatMessageBlock};
use url::Url;
...
/// `chat_message_from_composer_text` 将 composer 文本归一化为带附件块的 `ChatMessage`。
...
mod tests {
    ...
    #[test]
    fn composer_message_builder_embeds_supported_files_into_blocks() {
        let root = temp_root("composer-message-rich");
        ...
        let message =
            chat_message_from_composer_text("review @assets/sample.png @src/code.py", &root);
    ...
    #[test]
    fn composer_message_builder_leaves_unsupported_binary_as_plain_text() {
        let root = temp_root("composer-message-unsupported");
        fs::write(root.join("archive.zip"), [0x50, 0x4b, 0x03, 0x04]).expect("write zip fixture");

        let message = chat_message_from_composer_text("inspect @archive.zip", &root);
    ...
    #[test]
    fn composer_message_builder_embeds_utf8_text_without_known_extension() {
        let root = temp_root("composer-message-extensionless");
        fs::write(root.join("Dockerfile"), b"FROM rust:1.89\n").expect("write dockerfile");

        let message = chat_message_from_composer_text("inspect @Dockerfile", &root);
