...
impl MessageItem {
    ...
    pub fn render_lines(&self, width: u16, palette: TerminalPalette) -> Vec<Line<'static>> {
        match self.sender {
            ...
            Sender::Assistant => render_assistant_message(
                assistant_display_content(self.content.as_ref()),
                width,
                palette,
            ),
        }
    ...
    pub(crate) fn measure_render_metrics(
        ...
        match self.sender {
            ...
            Sender::Assistant => render_assistant_message_metrics(
                assistant_display_content(self.content.as_ref()),
                width,
                palette,
            ),
        ...
        (self.sender == Sender::Assistant)
            .then(|| {
                assistant_projection::render_assistant_message_projection(
                    assistant_display_content_rc(&self.content),
                    width,
                    palette,
                )
            })
...
fn assistant_display_content(content: &str) -> &str {
...
fn assistant_display_content_rc(content: &Rc<str>) -> Rc<str> {
    let display_content = assistant_display_content(content.as_ref());
