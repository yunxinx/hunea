...
pub(crate) fn stage_message_history_recall(model: &mut Model, text: &str) {
    let _ = model.blind_recall.push_local_entry(text);
}

/// 写入盲回溯并返回异步持久化 effect（清空输入、picker Enter 等路径）。
pub(crate) fn commit_message_history(model: &mut Model, text: &str) -> Option<AppEffect> {
    let text = model.blind_recall.push_local_entry(text)?;
