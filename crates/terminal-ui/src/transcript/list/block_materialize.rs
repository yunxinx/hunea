...
use super::*;
use crate::styled_text::{line_plain_text_len, line_to_plain_text};
...
pub(crate) fn materialize_transcript_item_render_block(
    ...
    let lines = match item {
        TranscriptItem::ToolResult(tool_result) => {
            let marker_now = if motion_mode.allows_animation() {
                context.now()
            } else {
                tool_result
            ...
            tool_result.render_lines_at(width, palette, marker_now)
        }
...
impl TranscriptItem {
    ...
    pub(super) fn measure_render_metrics(
        ...
        match self {
            Self::StartupBanner(item) => item.measure_render_metrics(width, palette),
            ...
            Self::AgentFact(item) => item.measure_render_metrics(width, palette),
        }
    }

    pub(super) fn render_lines(&self, width: u16, palette: TerminalPalette) -> Vec<Line<'static>> {
        match self {
            Self::StartupBanner(item) => item.render_lines(width, palette),
            ...
            Self::AgentFact(item) => item.render_lines(width, palette),
        }
    }
    ...
    pub(super) fn render_for_terminal_replay(
        ...
        match self {
            ...
            Self::AgentFact(item) => item.render_for_terminal_replay(width, palette, preserve_ansi),
        }
    }

    pub(super) fn render_plain_text(&self, width: u16, palette: TerminalPalette) -> String {
        match self {
            Self::StartupBanner(item) => item.render_plain_text(width, palette),
            ...
            Self::AgentFact(item) => item.render_plain_text(width, palette),
        }
    }
    ...
    pub(super) fn render_line_anchors(
        ...
        match self {
            Self::StartupBanner(item) => item.render_line_anchors(width, palette),
            ...
            Self::AgentFact(item) => item.render_line_anchors(width, palette),
        }
    }
    ...
    pub(crate) fn render_cache_key(&self) -> u64 {
        match self {
            Self::StartupBanner(item) => item.render_cache_key(),
            ...
            Self::AgentFact(item) => item.render_cache_key(),
        }
    }

    pub(crate) fn source_text_byte_len(&self) -> usize {
        match self {
            Self::StartupBanner(item) => item.source_text_byte_len(),
            ...
            Self::AgentFact(item) => item.source_text_byte_len(),
        }
    }
