...
use crate::{
    ...
    startup_banner::StartupBannerItem,
...
pub(crate) enum TranscriptItem {
    StartupBanner(StartupBannerItem),
    Message(MessageItem),
...
impl Transcript {
    ...
    /// `append_startup_banner` 追加一条启动欢迎块项。
    pub(crate) fn append_startup_banner(&mut self, options: StartupBannerOptions) {
        self.push_item(TranscriptItem::StartupBanner(StartupBannerItem::new(
            options,
        )));
    }
