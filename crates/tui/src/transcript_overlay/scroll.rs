...
impl Model {
    /// 计算 transcript 中 Hero item 占用的行数；Hero 不进入覆盖层。
    pub(super) fn transcript_overlay_hero_lines_for_index(
        &self,
        ...
        if matches!(first_item.as_ref(), TranscriptItem::Hero(_)) {
    ...
    pub(super) fn transcript_overlay_max_offset_for_index(
        ...
        let effective_total = index.line_count.saturating_sub(hero_lines);
