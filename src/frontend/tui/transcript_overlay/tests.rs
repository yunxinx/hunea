...
#[test]
fn overlay_initial_scroll_subtracts_hero_lines_from_document_offset() {
    let mut model = Model::new(HeroOptions::default());
    ...
    let hero_lines = model.transcript_overlay_hero_lines_for_index(&index);
    assert!(
        hero_lines > 0,
        "default transcript should include a Hero item"
    );
    model.document_runtime.viewport_y = hero_lines + 2;
    ...
    assert_eq!(
        model.transcript_overlay.as_ref().unwrap().scroll_offset,
        2,
        "opening overlay from the main transcript viewport should skip Hero lines"
    );
}
...
#[test]
fn overlay_ignores_mouse_wheel_events_to_preserve_native_mouse_policy() {
    let mut model = Model::new(HeroOptions::default());
    model.set_window(20, 10);
    model.set_palette(default_palette(), true);
    model.transcript_mut().clear();
