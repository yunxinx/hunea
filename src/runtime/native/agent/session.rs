...
use crate::runtime::native::NativeLlmPerformanceMetrics;

/// `NativeAgentRuntimeState` 管理内置 native agent 请求的后台 worker 与取消状态。
#[derive(Default)]
pub(crate) struct NativeAgentRuntimeState {
    pub(crate) receiver: Option<Receiver<NativeAgentEvent>>,
...
impl NativeAgentRuntimeState {
    ...
    pub(crate) fn try_recv_event(&mut self) -> Option<NativeAgentEvent> {
...
pub(crate) enum NativeAgentEvent {
    Retrying {
        message: String,
    },
    OutputTokenEstimate {
...
mod tests {
    ...
    use crate::runtime::{
        native::{ChatMessage, NativeAgentRequest, ProviderKind},
        request_policy::RuntimeRequestPolicy,
        session::RuntimeTarget,
