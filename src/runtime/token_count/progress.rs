...
#[derive(Debug, Clone)]
pub(crate) struct StreamingTokenProgress {
    model_id: String,
    pending_text: String,
    total_tokens: usize,
...
impl StreamingTokenProgress {
    ...
    pub(crate) fn observe_delta(&mut self, delta: &str, now: Instant) -> Option<usize> {
        if delta.is_empty() {
            return None;
        }
    ...
    pub(crate) fn flush(&mut self, now: Instant) -> Option<usize> {
